...
function appendSegment(merged, clip, offset) {
	...
	for (const track of clip.tracks) {
		...
		// Local fix: morph weights have one value per key, unlike vectors.
		const stride = track.type === 'quaternion' ? 4 : track.type === 'number' ? 1 : 3;
	...
	for (const [name, lane] of merged) {
		...
		const stride = lane.type === 'quaternion' ? 4 : lane.type === 'number' ? 1 : 3;
		lane.times.push(end);
		lane.values.push(...lane.values.slice(-stride));
	}
	return end;
